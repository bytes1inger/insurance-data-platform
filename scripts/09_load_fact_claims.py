"""
fact_claims ETL Pipeline
========================
Pattern: Extract → Validate → Transform → Deduplicate → Load → Verify
Goal: Zero data loss + full reconciliation with claims table
"""
...
def extract():
    ...
    policy_types = pd.read_sql("SELECT policy_type_id, policy_type FROM dim_policy_type", engine)  # ✅ added
...
def validate(claims):
    ...
    # Date logic — claim_date should be AFTER incident_date
...
def transform(claims, customers, policies, counties, statuses, policy_types):
    ...
    # -----------------------------
    # JOIN 1: customer → get county
    # -----------------------------
    df = df.merge(customers, on="customer_id", how="left")

    # -----------------------------
    # JOIN 2: county name → county_id  ✅ FIXED (county comes from dim_customer)
    # -----------------------------
    df = df.merge(
        counties,
        left_on="county",       # ✅ county column from dim_customer join above
        right_on="county_name",
        how="left"
    )

    # -----------------------------
    # JOIN 3: policy → get policy_type
    # -----------------------------
    df = df.merge(policies, on="policy_id", how="left")

    # -----------------------------
    # JOIN 4: policy_type → policy_type_id  ✅ FIXED (was missing)
    # -----------------------------
    df = df.merge(policy_types, on="policy_type", how="left")

    # -----------------------------
    # JOIN 5: claim_status → status_id
    # -----------------------------
    df = df.merge(
        statuses,
        left_on="claim_status",
    ...
    if unmatched_status  > 0: log.warning(f"  ⚠️  Unmatched statuses     : {unmatched_status}")
    if unmatched_policy  > 0: log.warning(f"  ⚠️  Unmatched policy types : {unmatched_policy}")
    if unmatched_county  > 0: log.warning(f"  ⚠️  Unmatched counties     : {unmatched_county}")

    # -----------------------------
    # BUILD FINAL FACT STRUCTURE
    # -----------------------------
    fact = pd.DataFrame({
        "claim_id":       df["claim_id"].astype(str),
        "policy_id":      df["policy_id"].astype(str),
        "customer_id":    df["customer_id"].astype(str),
        "date_key":       df["claim_date"],             # ✅ links to dim_date
        "incident_date_key": df["incident_date"],       # ✅ links to dim_date
        "status_id":      df["status_id"],
        "county_id":      df["county_id"],
        "policy_type_id": df["policy_type_id"],         # ✅ now correctly mapped
...
def deduplicate(df):
    ...
    try:
        ...
        before   = len(df)
        df       = df[~df["claim_id"].astype(str).isin(existing["claim_id"].astype(str))]
...
def verify():
    ...
    SELECT COUNT(*)
    FROM claims c
    LEFT JOIN fact_claims f ON c.claim_id::text = f.claim_id::text
    WHERE f.claim_id IS NULL
""")).scalar()
        null_amounts = conn.execute(text(
            "SELECT COUNT(*) FROM fact_claims WHERE claim_amount IS NULL"
        )).scalar()
    ...
    if missing == 0 and null_amounts == 0:
        ...
        log.warning("  ⚠️  DATA QUALITY ISSUES DETECTED — investigate above")
